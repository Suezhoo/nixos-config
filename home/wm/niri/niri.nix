{
  ...
  xdg.configFile."niri/config.kdl".text = ''
      ...
      // Generated by NixOS configurationV
    ...
    input {
        ...
        // Focus fully visible windows and outputs as the pointer moves over them,
        // without scrolling the workspace unexpectedly.
        focus-follows-mouse
        // max-scroll-amount="0%"
    }
    ...
    // Global quality settings for background blur requested by clients
    // such as Kitty or enabled through window/layer rules.  More passes
    // increase smoothness at some GPU cost; offset increases blur spread.
        blur {
            passes 2
            offset 1.65
            noise 0.01
    ...
    // Application-specific behavior lives in its own declarative KDL file.
    ...
    binds {
        ...
        // Save the currently running OBS replay buffer, matching the global
        // shortcut configured for the KineticWE session.
        F8 repeat=false allow-inhibiting=false hotkey-overlay-title="Save OBS Replay" {
            spawn "${config.home.profileDirectory}/bin/obs-save-replay";
        }
        ...
        // Navigate between physical monitors with the mouse thumb buttons.
        Mod+MouseForward   { focus-monitor-left; }
        Mod+MouseBack { focus-monitor-right; }
